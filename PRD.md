...
1. **Backtest (Gate 1, "proves the edge").** 2 to 3 years of data, real costs (fees, funding, slippage, minimum notional).
   ...
   **Harness built and run end to end on real data; Gate 1 NOT PASSED.**
   ...
   The exit-side parameters were subsequently investigated as one such candidate (`backtest_reports/retune_2026-08-22.md`) and ruled out: the momentum-shift exit that dominates the exit mix is protective, not premature (following the 21 out-of-sample exits forward on their own bracket, 17 of 21 would have ridden down to a full stop and holding would have worsened the result from -0.069R to -0.456R on those trades), the trailing stop is structurally inert because out-of-sample trades never reach the +1R it arms at, and six hypothesis-driven exit-parameter variants all land within 0.001R of the failing -0.163R baseline with an identical signal count, so no exit-parameter change clears Gate 1 and the current exit parameters are kept.
   That leaves the two live constraints as entry predictiveness and the structural signal-count shortfall, neither an exit-parameter matter.
   The "expand the universe further" candidate has now also been executed, again without touching `StrategyConfig` (`backtest_reports/universe_expansion_2026-08-23.md` and `.json`; see [AGENTS.md](AGENTS.md), "Architecture decisions from the universe expansion").
   A rule-defined universe selector was built (`crypto_trader.ingest.universe`: a 24h quote-volume floor of 2,000,000 USDT plus a 50,000-USDT order-book depth check within +-0.5% of mid, run against Bitunix public market data), yielding 60 genuinely liquid symbols on 2026-08-23 (from 724 listed pairs), and the sample was widened two ways: Run A ingested all 60 from Bitunix's own klines (2022-2026), and Run B added a research-only deep-history source (Binance spot via the public data-vision mirror, since Binance's futures API is geo-blocked here) covering the 45 of 60 symbols Binance lists, back to 2017.
   The result: the negative edge does not just survive the larger sample, it hardens into statistical significance.
   Run A reaches 199 out-of-sample closed trades (clearing the 150-300 count target for the first time) at -0.239R with a 95% CI of [-0.342, -0.125], and Run B agrees across a 2017-2026 multi-regime window (largest combination -0.243R over 625 out-of-sample trades, CI [-0.330, -0.157]); every swept combination in both runs now has an out-of-sample confidence interval that excludes zero on the NEGATIVE side.
   So the structural signal-count shortfall is now RESOLVED (ample n at larger N) and entry predictiveness is CONFIRMED as the single binding constraint; the remaining candidate next steps (a materially different entry rule, a different strategy, or accepting that this parameterization does not clear Gate 1) are a captain decision under captain decision 5, not a unilateral edit.
...
- History supplement from a major venue is permitted if Bitunix's own depth is shallow, but research/backtest only, never for live decisions.
  This is now built and used: `crypto_trader.ingest.binance_source.BinanceSpotCandleSource` reads Binance spot via the public data-vision mirror (`data-api.binance.vision`, since Binance's futures API is geo-blocked from the build environment) for the universe-expansion Gate 1 study, back to 2017 for the majors.
  It is RESEARCH ONLY and structurally isolated from every live/paper path, proven by `tests/test_research_source_isolation.py` (no module under `paper/`, `exchange/`, `safety/`, `approval/`, or `api/` may reference Binance), with a `RESEARCH_ONLY` marker on the class; its candles are written only to a separate research database, never the live store.
  Its OHLCV parity with Bitunix's own candles was measured before use (BTCUSDT 4H, ~9,500 overlapping bars, mean |diff| ~0.045% per field), closing the design-review parity risk below.
- The trading universe is now rule-defined and self-updating (`crypto_trader.ingest.universe`, a 24h volume floor plus an order-book depth check against Bitunix public market data), replacing the earlier hand-picked list; see section 6.2 and [AGENTS.md](AGENTS.md).
...
This second item is now addressed for the one supplemental source in use: the Binance-spot vs Bitunix-perp BTCUSDT 4H parity was measured at ~0.045% mean per-field difference before the deep-history run relied on it (see section 6.2 and the universe-expansion report), and that source is research-only and import-isolated from every live path.
